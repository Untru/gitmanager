...
Процедура УстановитьДоступностьЭлементов(ТекущиеНастройкиПользователя = Неопределено)
	...
	Если ДоступностьЭлементов = Истина Тогда
		Элементы.ЗафиксироватьИзменения.Доступность = РазрешеноФиксироватьИзмененеия(База, ТекущаяЗадача);
...
Процедура ПолучитьВеткиНаСервере()
	...
	ТипТаскТрекера = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Проект, "ТаскТрекер.ТипТаскТрекера");
	Если ТипТаскТрекера = Перечисления.ТаскТрекеры.Отсутствует Тогда
		ПоказыватьЧужиеЗадачи = Истина;
	КонецЕсли;
...
Функция КомандаЗапускаЗаявкаНаСлияние(НастройкиПользователя, База, ДополнительныеПараметры)
	...
	Если ЭтоГитхаб(ПараметрыБазы.Проект) Тогда
		КомандаЗапуска = КомандыЗапускаПриложения.НовыйКомандаЗаявкаНаСлияниеGitHub(ПараметрыБазы,
			НастройкиПользователя,
			ДополнительныеПараметры);
	Иначе
		КомандаЗапуска = КомандыЗапускаПриложения.НовыйКомандаЗаявкаНаСлияние(ПараметрыБазы,
			НастройкиПользователя,
			ДополнительныеПараметры);
	КонецЕсли;	
...
Процедура ОбновитьСписокЗадач()
	...
	СписокЗадачи.Сортировать("ТипЗадачи ВОЗР, Номер УБЫВ");
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция РазрешеноФиксироватьИзмененеия(База, ТекущаяЗадача)
	...
	Если ЭтоСлужебнаяВетка(ТекущаяЗадача) Тогда
		Возврат Ложь;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЭтоСлужебнаяВетка(ТекущаяЗадача)
	
	Возврат Не ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяЗадача, "ПрефиксВетки"));
